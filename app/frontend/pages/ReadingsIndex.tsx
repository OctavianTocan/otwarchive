...
function VisitMeta({ r }: { r: Reading }) {
  return (
    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-border border-t border-dashed pt-3 text-muted-foreground text-xs tabular-nums">
      {r.lastViewed && <span><span className="font-semibold">Last visited:</span> {r.lastViewed}</span>}
      <span>{r.viewCount === 1 ? "Visited once" : `Visited ${n(r.viewCount)} times`}</span>
      {r.markedForLater && <Badge variant="secondary">Marked for Later</Badge>}
    </div>
  );
}
...
function ReadingCard({ r }: { r: Reading }) {
  ...
  return (
    <Card className="px-5 rounded-none border-x-0 border-t-0 py-5 transition-colors last:border-b-0 hover:bg-muted/30">
      ...
      <dl className="flex flex-wrap gap-x-4 gap-y-0.5 text-muted-foreground text-xs tabular-nums">
...
export default function ReadingsIndex({ context, readings, pagination }: Props) {
