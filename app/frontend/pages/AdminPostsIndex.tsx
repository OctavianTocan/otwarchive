  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-[900px] px-4 pt-6 pb-16 md:px-5">
        <h2 className="mb-4 font-bold text-2xl">{context.heading}</h2>
        {posts.length === 0 && <p className="py-6 text-muted-foreground">No news posts yet.</p>}
        <div className="flex flex-col divide-y divide-border">
          {posts.map((p) => (
            <article key={p.id} className="py-6">
              <h3 className="font-semibold text-lg"><a href={p.url} className="text-link hover:underline">{p.title}</a></h3>
